...
function dataPayload(d){return JSON.stringify(d)}
...
function parseEnvelope(raw){const parsed=JSON.parse(raw);if(parsed&&parsed.data&&Number.isFinite(+parsed.schemaVersion)){
 const payload=dataPayload(parsed.data);if(parsed.checksum&&parsed.checksum!==fnv1a(payload))throw new Error('資料完整性檢查失敗');return {data:parsed.data,schemaVersion:+parsed.schemaVersion}}
 return {data:parsed,schemaVersion:0}
}
function isUsableData(d){return !!d&&Array.isArray(d.tasks)&&Array.isArray(d.logs)}
function persistEnvelope(d,{backup=true}={}){
 const normalized=normalize(d);normalized.activities=mergeActivityCatalog(normalized.activities);normalized.scholarships=mergeScholarshipCatalog(normalized.scholarships);if(!Array.isArray(normalized.activities)||!normalized.activities.length)normalized.activities=ACTIVITY_SEED.map(x=>({...x}));if(!Array.isArray(normalized.scholarships)||!normalized.scholarships.length)normalized.scholarships=SCHOLARSHIP_SEED.map(x=>({...x}));normalized.schoolCalendar=Array.isArray(normalized.schoolCalendar)?normalized.schoolCalendar:[];
...
let calendarCursor=new Date(),selected=null,timer={id:null,start:0,elapsed:0,running:false},goalPath={long:null,mid:null,short:null,exec:null};
...
function seed(){const d={tasks:[
 ...
 mk('g3-6','TOEIC 正式考試',3,'g3','未開始',0,'不定期'),
 mk('g3-6-1','2026/12/20 TOEIC 聽力與閱讀測驗',4,'g3-6','未開始',0)],logs:[],activities:ACTIVITY_SEED.map(x=>({...x})),scholarships:SCHOLARSHIP_SEED.map(x=>({...x})),schoolCalendar:THU_SCHOOL_CALENDAR_BASE.map((e,i)=>({...e,id:e.id||('thu115-'+i)})),calendarEvents:[],executionPlans:[]};
...
function migrateData(d,fromVersion=0){
 ...
 let out=d;
 if(fromVersion<5) out=normalize(out);
...
function ensureToeicPlan(){
 const add=[
  ...
  ['g3-5','考前衝刺',2,'g3',0],['g3-5-1','完整模擬與考前調整',3,'g3-5',0],['g3-5-1-1','完整模擬測驗',4,'g3-5-1',150],['g3-5-1-2','閱讀速度與時間配置',4,'g3-5-1',90],['g3-5-1-3','聽力穩定度與最後修正',4,'g3-5-1',90],['g3-6','TOEIC 正式考試',3,'g3',0],['g3-6-1','2026/12/20 TOEIC 聽力與閱讀測驗',4,'g3-6',0]
 ];
 ...
 const periods={'g3-1-1':['2026-09-07','2026-09-27'],'g3-2-1':['2026-09-28','2026-10-25'],'g3-2-2':['2026-09-28','2026-10-25'],'g3-3-1':['2026-11-03','2026-11-09'],'g3-4-1':['2026-11-10','2026-12-06'],'g3-5-1':['2026-12-07','2026-12-19'],'g3-6':['2026-12-20','2026-12-20']};
...
function actualMinutesInRange(t,start,end){
 ...
 return db.logs.filter(x=>x.actual&&String(x.taskId)===String(t.id)).reduce((sum,x)=>{const day=logDate(x.time);if(!day||day<start||day>end)return sum;const p=periodForTask(t);if(!inPeriod(day,p))return sum;return sum+Math.max(0,+x.minutes||0)},0);
}
...
function openGoalInfoModal(id,autoSchedule=false){
 ...
 ${t.level===4?`<div class="planned-info"><div class="planned-info-head"><b>已安排執行</b><span>${(Array.isArray(db.executionPlans)?db.executionPlans.filter(x=>String(x.taskId)===String(t.id)&&x.status!=='已完成').length:0)} 次</span></div><div class="planned-info-list">${(Array.isArray(db.executionPlans)?db.executionPlans.filter(x=>String(x.taskId)===String(t.id)&&x.status!=='已完成').sort((a,b)=>(a.date+' '+a.time).localeCompare(b.date+' '+b.time)).slice(0,4):[]).map(x=>`<div class="planned-info-item"><span>${esc(x.date)} ${esc(x.time)}</span><b>${esc(x.minutes)} 分</b></div>`).join('')||'<div class="muted" style="font-size:11px">尚未安排</div>'}</div></div>`:''}
...
function stats(){
 ...
 const target=activeWeeklyTargetTotal();
 const actual=db.logs.filter(l=>l.actual!==false).reduce((s,l)=>s+(+l.minutes||0),0);
 const weekActual=db.logs.filter(l=>l.actual!==false&&weekStartKey(l.time?.slice(0,10)||'')===weekStartKey(todayKey())).reduce((s,l)=>s+(+l.minutes||0),0);
 ...
 document.getElementById('est').textContent=target;
 document.getElementById('logsN').textContent=db.logs.length;
 ...
 const logs=db.logs.filter(l=>l.actual!==false).slice(0,6);
 document.getElementById('recentLogs').innerHTML=logs.length?logs.map(l=>`<div class="listitem"><div><b>${esc(l.name||getTask(l.taskId)?.name||'未命名行動')}</b><small class="muted" style="display:block">${esc((l.time||'').slice(0,16).replace('T',' · '))}</small></div><span>${+l.minutes||0} 分</span></div>`).join(''):'<div class="empty">尚無實際投入紀錄</div>'; renderWeeklyReview();
}
...
function closeTransientOverlays(){try{closeEditModal()}catch(e){}try{closeGoalInfoModal()}catch(e){}try{closeScholarshipInfoModal()}catch(e){}try{closeModal()}catch(e){}try{closeTest()}catch(e){}document.body.style.overflow='';}
...
function selectTodayExecution(id){const t=getTask(id);if(!t)return;if(timer.running){toast('目前已有計時進行中，請先完成或暫停');return}timer.id=id;timer.elapsed=0;timer.running=false;timer.start=0;document.getElementById('timerTask').textContent='待執行：'+t.name;document.getElementById('timerTaskCancel').style.display='block';updateClock();updateTimerButtons();today();toast('已選取今日欲執行項目；尚未開始計時')}
function cancelTodaySelection(){if(timer.running){toast('計時進行中，請先暫停或完成後再取消');return}timer={id:null,start:0,elapsed:0,running:false};document.getElementById('timerTask').textContent='尚未選擇任務';updateClock();updateTimerButtons();today();toast('已取消今日執行選取')}
...
function openTodayExecution(id){go('today');selectTodayExecution(id)}
function useTimer(id){selected=id;timer.id=id;timer.elapsed=0;timer.running=false;timer.start=0;document.getElementById('timerTask').textContent=getTask(id)?.name||'';go('today');updateClock();updateTimerButtons();toast('已選擇具體行動，可開始計時')}
...
function finishTimer(){if(!timer.id){toast('請先選擇具體行動');return}if(timer.running){timer.elapsed+=Date.now()-timer.start;timer.running=false}const t=getTask(timer.id);if(!t){toast('找不到目前執行的具體行動');return}const mins=Math.max(1,Math.round(timer.elapsed/60000));db.logs.unshift({id:'log'+Date.now()+Math.random(),taskId:t.id,name:t.name,time:new Date().toISOString(),minutes:mins,actual:true});save();const done=timer.id;timer={id:null,start:0,elapsed:0,running:false};renderAll();document.getElementById('timerTask').textContent='已完成：'+(t.name||done);document.getElementById('timerTaskCancel').style.display='none';updateClock();updateTimerButtons();toast(`已記錄 ${mins} 分鐘；完成度已更新`)}
...
function overviewMonthItems(date=todayKey()){
 ...
 (Array.isArray(db.calendarEvents)?db.calendarEvents:[]).forEach(e=>add(e.date,e.title,e.meta||'行事曆',e.type||'calendar'));
...
function weeklyWorkload(date=todayKey()){
 ...
 const actual=db.logs.filter(x=>x.actual).reduce((s,x)=>{const d=logDate(x.time);const ws=weekStartKey(date),we=weekEndKey(date);return s+(d>=ws&&d<=we?(+x.minutes||0):0)},0);
...
function dashboard(){
 ...
 const weekly=activeWeeklyTargetTotal();document.getElementById('dRun').textContent=(weekly/60).toFixed(1).replace('.0','')+'h';
 document.getElementById('dMin').textContent=db.logs.filter(x=>x.actual).reduce((s,x)=>s+(+x.minutes||0),0);
...
function renderPlannedQueue(){
 ...
 const plans=(Array.isArray(db.executionPlans)?db.executionPlans:[]).filter(x=>x.status!=='已完成').sort((a,b)=>(String(a.date)+' '+String(a.time)).localeCompare(String(b.date)+' '+String(b.time)));
 const today=todayKey();
 if(count)count.textContent=plans.length?`${plans.length} 項`:'尚無安排';
 box.innerHTML=plans.slice(0,8).map(x=>{const isToday=x.date===today;return `<div class="listitem planned-row"><div><b>${esc(x.name||getTask(x.taskId)?.name||'未命名行動')}</b><small class="muted" style="display:block">${esc(x.date)} ${esc(x.time)} · ${esc(x.minutes)} 分鐘${isToday?' · 今日':''}</small></div><div class="planned-actions"><span class="planned-status">${isToday?'待執行':'已安排'}</span><button class="btn" type="button" onclick="openTodayExecution('${esc(x.taskId)}')">選取</button></div></div>`}).join('')||'<div class="empty">尚未安排未來執行項目。可在「目標地圖 → 具體實現方式 → 安排執行」建立。</div>';
}
function getTodayItems(){
 const today=todayKey();
 const plannedIds=new Set((Array.isArray(db.executionPlans)?db.executionPlans:[]).filter(x=>x.date===today&&x.status!=='已完成').map(x=>String(x.taskId)));
...
function exportDB(){
 const envelope=makeEnvelope(normalize(db));const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(envelope,null,2)],{type:'application/json'}));a.download=`個人目標與學習行動管理系統_${APP_VERSION}_備份.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}
function importDB(){
 const i=document.createElement('input');i.type='file';i.accept='.json,application/json';i.onchange=()=>{const file=i.files?.[0];if(!file)return;const r=new FileReader();r.onload=()=>{
  const previous=db;
  try{
   const parsed=parseEnvelope(String(r.result));
...
function activityAutoClass(a){const allowed=['法律／學術','語言／國際','教育／青少年','職涯／實習','公共參與','獎學金／助學金'];if(allowed.includes(a.type))return a.type;const s=(a.title+' '+(a.keywords||'')).toLowerCase();if(/法律|憲法|民法|刑法|學術/.test(s))return'法律／學術';if(/英語|日語|語言|國際|海外|交換/.test(s))return'語言／國際';if(/青少年|教育|教學|課程|帶領/.test(s))return'教育／青少年';if(/獎學金|助學金|獎助學金|獎勵金|助學/.test(s))return'獎學金／助學金';if(/工作|實習|打工|履歷|職涯/.test(s))return'職涯／實習';return a.type||'公共參與'}
function activityMatchDetails(a){
 ...
 tasks.forEach(t=>{
   ...
   const corpus=name+' '+path;
   if(!corpus.trim())return;
   const weight={1:2,2:4,3:7,4:10}[Number(t.level)]||2;
   let z=0;
   ...
   if(a.type==='獎學金／助學金'&&/獎學金|助學金|獎助|清寒|優秀學生|學費|助學/.test(text))z+=weight*3;
   if(z>bestScore){bestScore=z;best=t}
 });
 return {task:best,matchScore:bestScore};
}
function activityFit(a){
 const rel=activityMatchDetails(a),d=activityDistance(a.scope),text=(a.title+' '+(a.keywords||'')).toLowerCase();
 const goal=Math.min(45,Math.round(rel.matchScore*3));
 const direct=a.direct===true&&!a.team;
 const knowledge=/獎學金|助學金|獎助|實習|工作|打工|職涯|學術|法律|教育|語言|國際|技能|培訓|講座|見習/.test(text)?20:10;
 const geo=Math.max(0,10-(d-1));
 const freshness=a.available===false?0:5;
 const score=Math.max(0,Math.min(100,goal+(direct?20:0)+knowledge+geo+freshness));
 return {score,goal,direct:direct?20:0,knowledge,geo,freshness,task:rel.task,eligible:activityIsEligible(a)};
}
function clearActivitySearch(){const input=document.getElementById('activitySearch');if(input)input.value='';updateActivitySearchUI();renderActivities();toast('已清除搜尋')}
function bindActivitySearch(){const e=document.getElementById('activitySearch');if(e&&!e.dataset.bound){e.dataset.bound='1';e.addEventListener('input',updateActivitySearchUI)}}
function applyActivitySearch(){bindActivitySearch();const b=document.getElementById('activitySearchButton');if(b){b.classList.add('searching');setTimeout(()=>b.classList.remove('searching'),180)}renderActivities();updateActivitySearchUI();toast('已套用活動搜尋')}
function activityIsEligible(a){return a.direct!==false&&!a.team&&a.available!==false}
...
function normalizeRemoteActivity(a,i){
 ...
 x.title=String(x.title||'未命名活動').trim();
 x.date=String(x.date||'').trim(); x.time=String(x.time||'').trim();
 x.scope=String(x.scope||'全臺').trim(); x.type=String(x.type||'公共參與').trim();
 x.kind=x.kind||'event';
 x.scholarship=!!x.scholarship; x.url=String(x.url||x.externalUrl||'').trim();
 x.keywords=String(x.keywords||'').trim(); x.source=String(x.source||'自動活動資料').trim();
 x.statusText=String(x.statusText||'自動抓取').trim();
 x.direct=x.direct!==false; x.team=!!x.team; x.available=x.available!==false; if(x.scholarship)x.type='獎學金／助學金';
 return x;
}
function hashCode(str){let h=0;for(let i=0;i<str.length;i++)h=((h<<5)-h)+str.charCodeAt(i)|0;return h}
function mergeRemoteActivities(base,remote){
 const map=new Map((Array.isArray(base)?base:[]).map(a=>[String(a.id),a]));
 (Array.isArray(remote)?remote:[]).forEach((a,i)=>{const x=normalizeRemoteActivity(a,i);map.set(String(x.id),{...(map.get(String(x.id))||{}),...x})});
 return [...map.values()];
}
async function loadRemoteActivities(){
 try{
  ...
  const normalizedItems=items.map(normalizeRemoteActivity);
  const normalizedScholarships=scholarshipItems.map(normalizeRemoteActivity);
  const remoteScholarships=[...normalizedItems.filter(x=>x.scholarship||x.type==='獎學金／助學金'||x.kind==='scholarship'),...normalizedScholarships];
  remoteActivityCatalog=normalizedItems.filter(x=>!x.scholarship&&x.type!=='獎學金／助學金'&&x.kind!=='scholarship');
  activityAutoMeta=payload.meta||{};
  db.scholarships=mergeScholarshipCatalog([...(Array.isArray(db.scholarships)?db.scholarships:[]),...remoteScholarships]);
  ...
  if(box){box.style.display='block';const u=document.getElementById('activityAutoUpdated');const m=document.getElementById('activityAutoMeta');if(u)u.textContent=activityAutoMeta.updatedAt?('更新 '+new Date(activityAutoMeta.updatedAt).toLocaleString('zh-TW',{hour12:false})):'自動資料';if(m)m.textContent=`來源 ${activityAutoMeta.sources||0} · 活動 ${items.length} · 獎學金 ${scholarshipItems.length} · 成功 ${activityAutoMeta.ok||0} · 失敗 ${activityAutoMeta.failed||0}`}
 }catch(e){
  const box=document.getElementById('activityAutoStatus');if(box){box.style.display='block';const u=document.getElementById('activityAutoUpdated');const m=document.getElementById('activityAutoMeta');if(u)u.textContent='自動資料暫不可用';if(m)m.textContent='目前使用內建活動資料；下次更新會再嘗試。'}
 }
}
function renderActivities(){
 ...
 arr.sort((a,b)=>((a.date||'9999-12-31').localeCompare(b.date||'9999-12-31'))||b.fit.score-a.fit.score);
 const upcoming=arr.filter(a=>!a.date||a.date>=today).length;
 document.getElementById('activityStats').innerHTML=`目前 <b>${arr.length}</b> 項 · 可追蹤 ${upcoming} 項 · 已依「個人可參加／知識與專業／目標關聯／距離優先」完成適配度檢驗。`;
 const st=document.getElementById('activitySearchStatus');if(st)st.textContent=q?`搜尋「${q}」：找到 ${arr.length} 項`:`目前顯示 ${arr.length} 項符合判準的活動／計畫`;
 document.getElementById('activityList').innerHTML=arr.map(a=>{const f=a.fit;const relation=a.related?`<span class="activity-chip">→ ${esc(a.related.name)}</span>`:'';const dateBadge=a.date?(a.date<today?'<span class="activity-chip">已過期</span>':'<span class="activity-chip">可安排</span>'):'<span class="activity-chip">計畫資訊</span>';return `<div class="activity-card ${f.score>=80?'recommended':''}"><div class="activity-head"><div><h3>${esc(a.title)}</h3><div class="muted">${activityDateLabel(a,today)}</div></div><span class="activity-score">${f.score}%</span></div><div class="activity-meta"><span class="activity-chip">${esc(a.scope)}</span><span class="activity-chip">${esc(a.type)}</span>${relation}${dateBadge}</div><div class="activity-fit"><b>適配度</b><span>目標 ${f.goal}/45</span><span>個人參加 ${f.direct}/20</span><span>知識／專業 ${f.knowledge}/20</span><span>距離 ${f.geo}/10</span><span>有效性 ${f.freshness}/5</span></div><div class="activity-actions">${activityExternalUrl(a)?`<a class="btn" href="${esc(activityExternalUrl(a))}" target="_blank" rel="noopener noreferrer">外部資訊</a>`:''}${((a.kind==='event'||a.scholarship)&&a.date)?`<button class="btn gold" type="button" onclick="addActivityToCalendar('${a.id}')">${a.scholarship?'加入申請截止日':'加入行事曆'}</button>`:''}</div></div>`}).join('')||'<div class="empty">目前沒有符合條件的活動。</div>';
...
function openScholarshipInfo(id){
 ...
 <div class="goal-info-actions">${activityExternalUrl(a)?`<a class="btn primary" href="${esc(activityExternalUrl(a))}" target="_blank" rel="noopener noreferrer">查看官方資訊</a>`:''}${a.date?`<button class="btn dark" type="button" onclick="addActivityToCalendar('${a.id}');closeScholarshipInfoModal()">加入申請截止日</button>`:''}<button class="btn" type="button" onclick="closeScholarshipInfoModal()">關閉</button></div>`;
...
function renderActivityReferences(){
 ...
 box.innerHTML=refs.length?`<div class="reference-title">📚 相關計畫資料（不列入可直接參加活動）</div>`+refs.map(a=>`<div class="reference-item"><div><b>${esc(a.title)}</b><small>${esc(a.statusText||'參考資料')} · ${esc(a.source||'官方來源')}</small></div><a class="btn" href="${esc(activityExternalUrl(a))}" target="_blank" rel="noopener noreferrer">查看官方資訊</a></div>`).join(''):'';
}
function activityResetFilters(){['activitySearch'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});['activityScope','activityType'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='全部'});renderActivities()}
...
function calendarDetailHTML(title,subtitle,events){
  ...
  const body=list.length?`<div class="calendar-detail-list">${list.map(e=>{
    ...
    return `<div class="calendar-detail-item ${manual?'manual':(isAct?'activity':'school')}"><span class="dot"></span><div><b>${esc(e.title||'未命名行程')}</b><small>${esc(e.date||'')}${e.time?' · '+esc(e.time):''}${e.location?' · '+esc(e.location):''} · ${esc(e.meta|| (isAct?'活動':'學校行事'))}</small>${e.note?`<small>${esc(e.note)}</small>`:''}${e.score?`<span class="calendar-detail-reco">推薦 ${e.score} 分</span>`:''}</div><div class="detail-actions">${isAct&&e.url?`<a class="btn" href="${esc(e.url)}" target="_blank" rel="noopener">外部資訊</a>`:''}${manual?`<button class="btn" type="button" onclick="deleteCalendarManualEvent('${e.id}')">刪除</button>`:''}</div></div>`;
...
function renderAll(){db=normalize(db);removeLegacyStandaloneToeicGoals();ensureActivities();ensureToeicPlan();recalcAllStatuses();renderTree();dashboard();today();renderActivities();renderScholarships();renderCalendar();stats();bindInteractionFeedback();bindActivitySearch()}
...
function validateDB(){return validateData(db)}
...
function auditCoreModules(){
 const required=['go','renderAll','renderTree','renderActivities','renderScholarships','renderCalendar','selectTodayExecution','startTimer','finishExecution','save','exportDB','importDB','runSelfTest','clearCacheOnly'];
...
function runSelfTest(){
 ...
 try{const overseas=activityStore().find(a=>a.id==='a6'),fit=overseas&&activityFit(overseas);tests.push([!!fit&&fit.score>=70&&fit.task?.id==='g2','海外計畫適配度：正常']);const refs=document.querySelectorAll('#activityReferences .reference-item');tests.push([refs.length>=1,'非直接計畫分流：正常']);}catch(x){tests.push([false,'海外計畫適配度：'+x.message])}
...
function addActivityToCalendar(id){const a=catalogItemById(id);if(!a)return;db.calendarEvents=db.calendarEvents||[];if(!db.calendarEvents.some(e=>e.type==='activity'&&String(e.refId)===String(id))){db.calendarEvents.push({id:'ce'+Date.now()+Math.random(),type:'activity',refId:id,date:a.date,time:a.time||'',title:a.title,url:activityExternalUrl(a),meta:'已確認'})}save();renderCalendar();toast('已確認加入行事曆');}
renderAll();
