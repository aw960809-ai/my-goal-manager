...
payload={'activities':activities if isinstance(activities,list) else activities.get('events',[]),'scholarships':scholarships if isinstance(scholarships,list) else scholarships.get('scholarships',[]),'legacy':legacy,'meta':{'mode':'standalone-preview','version':'V96.5'}}
...
dev_modal='''<div class="developer-modal" id="developerModal" role="dialog" aria-modal="true" aria-labelledby="developerTitle" aria-hidden="true" onclick="if(event.target===this)closeDeveloperTools()"><div class="developer-box"><div class="settings-head"><div><h2 id="developerTitle">🧪 進階測試工具</h2><p>僅測試複本，不會寫入你的正式資料</p></div><button class="settings-close" type="button" onclick="closeDeveloperTools()">×</button></div><div class="developer-body"><div class="developer-banner"><b>V96.5</b><span>非破壞性資料生命週期測試</span></div><button class="btn gold developer-run" type="button" onclick="runStressTests()">執行完整壓力測試</button><div id="stressResult" class="stress-result"><div class="diagnostic-empty">尚未執行壓力測試。</div></div></div></div></div>'''
